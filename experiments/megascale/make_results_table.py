...
sys.path.append('..')
from get_full_table import METADATA_COLUMNS, HERMES_MODELS, PROTEINMPNN_MODELS, THERMOMPNN_MODELS


system_name = 'test_targets'
...
for dataset in ['overall']:
    ...
    for model in HERMES_MODELS + PROTEINMPNN_MODELS + THERMOMPNN_MODELS:
            
        try:
            if model in HERMES_MODELS + THERMOMPNN_MODELS:
    ...
    metadata_values = [f'megascale - {dataset}', False, '-ddG stability', num_measurements_trace[-1], num_structures_trace[-1], True, False, 'per structure' if dataset == 'per-structure' else 'overall']
