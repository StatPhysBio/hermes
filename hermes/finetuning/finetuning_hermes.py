...
def hermes_model_init_for_finetuning(model_dir, finetuning_params):
    ...
    if 'score_col' not in finetuning_params:
        finetuning_params['score_col'] = 'score'
...
def load_data(finetuning_params: Dict, hparams: Dict, data_irreps: o3.Irreps) -> Tuple[Dict[str, DataLoader], float, float]:
    ...
    score_col = finetuning_params['score_col']
    ...
    for split in ['train', 'valid', 'test']:
        targets = pd.read_csv(finetuning_params['targets_csv_template'].format(split=split, **finetuning_params))

        if split in ['train', 'valid']:
            # sample with replacement to make a better ensemble predictor
            targets = targets.sample(frac=1.0, replace=True, random_state = hash(str(hparams['seed'])+finetuning_params['score_col'])%(2**32-1)).reset_index(drop=True)

        if split == 'train':
            mean_train_score, std_train_score = targets[score_col].mean(), targets[score_col].std()

        dataset = FinetuningZernikegramsDataset(all_res_ids_to_zgrams, data_irreps, targets, score_col=score_col)
...
class FinetuningZernikegramsDataset(Dataset):
    def __init__(self, all_res_ids_to_zgrams: Dict, irreps: o3.Irreps, targets: pd.DataFrame, score_col: str = 'score'):
...
def loss_function(pred_logits_B20, target_aa_wt_idx_B, target_aa_mt_idx_B, target_score_B, device,
    ...
    alpha_gly = finetuning_params['alpha_gly']
    score_col = finetuning_params['score_col']
    ...
    if score_col in {'score_squared'}:
        pred_score_B = torch.square(pred_score_B)
        loss_reg = torch.nn.MSELoss()(pred_score_B, target_score_B)
    else:
        loss_reg = torch.nn.HuberLoss(delta=1.0)(pred_score_B, target_score_B)

    # compute loss whereby glycine is forced to have a zero score
    loss_gly = torch.nn.MSELoss()(pred_logits_B20[:, GLY_IDX], torch.zeros_like(pred_logits_B20[:, GLY_IDX]))

    return alpha_gly * loss_gly + loss_reg, loss_gly, loss_reg


def training_epoch(finetuning_params, model, optimizer, device, dataloader):
    model.train()

    loss_trace, loss_gly_trace, loss_reg_trace = [], [], []
    for zgram, aa_idx_wt, aa_idx_mt, score, res_id in tqdm(dataloader, total=len(dataloader)):
        ...
        loss, loss_gly, loss_reg = loss_function(pred_logits_B20, aa_idx_wt, aa_idx_mt, score, device, finetuning_params)

        loss.backward()
        optimizer.step()

        loss_trace.append(loss.item())
        loss_gly_trace.append(loss_gly.item())
    ...
    loss_gly_trace = loss_gly_trace[int(len(loss_gly_trace) * last_perc_to_keep):]
    loss_reg_trace = loss_reg_trace[int(len(loss_reg_trace) * last_perc_to_keep):]
    
    return np.mean(loss_trace), np.mean(loss_gly_trace), np.mean(loss_reg_trace)
...
def validation_epoch(finetuning_params, model, device, dataloader, return_logits_stats=False):
    ...
    if return_logits_stats:
        return np.mean(loss_trace), np.mean(loss_gly_trace), np.mean(loss_reg_trace), (np.mean(np.hstack(logsumexp_trace)), np.std(np.hstack(logsumexp_trace)))
    
    return np.mean(loss_trace), np.mean(loss_gly_trace), np.mean(loss_reg_trace)

def testing(model, device, dataloader, output_model_dir, return_scores=False):
    model.eval()
    ...
    if return_scores:
        return pr_results, sr_results, all_scores, all_pred_scores
    else:
...
def finetune_single_model(input_model_dir: str,
    ...
    if finetuning_params['lr_scheduler'] is None:
        ...
        if 'lr_scheduler_factor' not in finetuning_params:
            finetuning_params['lr_scheduler_factor'] = 0.5
        if 'lr_scheduler_patience' not in finetuning_params:
            finetuning_params['lr_scheduler_patience'] = 3
        lr_scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=finetuning_params['lr_scheduler_factor'], patience=finetuning_params['lr_scheduler_patience'], threshold=0.001, threshold_mode='rel', cooldown=0, min_lr=0, eps=1e-08)
    else:
        raise NotImplementedError()

    logfile = open(os.path.join(output_model_dir, 'log.txt'), 'w+')

    out_text = f'epoch\ttrain loss\tvalid loss\ttrain gly loss\tvalid gly loss\ttrain regr loss\tvalid regr loss\ttime (s)'
    print(out_text, flush=True)
    print(out_text, file=logfile, flush=True)

    start = time.time()
    train_loss, train_gly_loss, train_regr_loss = np.nan, np.nan, np.nan
    valid_loss, valid_gly_loss, valid_regr_loss, (orig_mean, orig_std) = validation_epoch(finetuning_params, model, device, dataloaders['valid'], return_logits_stats=True) # make this through the validation data just so it takes less time, it doesn't really matter it's just a rescaling in the ballpark
    ...
    out_text = f'0\t{train_loss:.4f}\t\t{valid_loss:.4f}\t\t{train_gly_loss:.4f}\t\t{valid_gly_loss:.4f}\t\t{train_regr_loss:.4f}\t\t{valid_regr_loss:.4f}\t\t{end-start:.2f}'
    print(out_text, flush=True)
    print(out_text, file=logfile, flush=True)
    ...
    for epoch in range(finetuning_params['n_epochs']):
        start = time.time()
        train_loss, train_gly_loss, train_regr_loss = training_epoch(finetuning_params, model, optimizer, device, dataloaders['train'])
        valid_loss, valid_gly_loss, valid_regr_loss = validation_epoch(finetuning_params, model, device, dataloaders['valid'])
        end = time.time()

        out_text = f'{epoch+1}\t\t{train_loss:.4f}\t\t{valid_loss:.4f}\t\t{train_gly_loss:.4f}\t\t{valid_gly_loss:.4f}\t\t{train_regr_loss:.4f}\t\t{valid_regr_loss:.4f}\t\t{end-start:.2f}'
        print(out_text, flush=True)
        print(out_text, file=logfile, flush=True)
    ...
    output = testing(model, device, dataloaders['test'], output_model_dir, return_scores=return_test_scores)

    pr_results, sr_results = output[0], output[1]
