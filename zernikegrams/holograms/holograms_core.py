...
def remove_zero_ns(l: int, ns: Union[List[int], np.ndarray]) -> np.ndarray:
    ...
    return ns_arr[np.logical_and(ns_arr >= l, ns_arr % 2 == (l % 2))]


def get_3D_zernike_function_indices(
    L_max: int,
    radial_nums: Union[List[int], np.ndarray],
    mode: str,
...
def zernike_coeff_lm_new(
    ...
    A = np.power(-1.0 + 0j, (n - l) / 2.0)

    B = np.sqrt(2.0 * n + D)
    C = sp.special.binom((n + l + D) // 2 - 1, (n - l) // 2)

    nl_unique_combs, nl_inv_map = np.unique(
        np.vstack([n, l]).T, axis=0, return_inverse=True
    )
    ...
    E_unique = sp.special.hyp2f1(
        -(n_hyp2f1_tile - l_hyp2f1_tile) / 2.0,
        (n_hyp2f1_tile + l_hyp2f1_tile + D) / 2.0,
        l_hyp2f1_tile + D / 2.0,
    ...
    lm_unique_combs, lm_inv_map = np.unique(
        np.vstack([l, m]).T, axis=0, return_inverse=True
    )
    ...
    y_unique = np.conj(
        sp.special.sph_harm(
            m_sph_harm_tile, l_sph_harm_tile, p[:num_lm_combs], t[:num_lm_combs]
        )
    )
    y = y_unique[lm_inv_map]

    if True in np.isinf(E):
        logger.warn("Error: E is inf")
        logger.warn(f"E={E}, n={n}, l={l}, D={D}, r={np.array(r)}, rmax={r_max}")
    # logger.debug(f"y: {y}")
    ...
    if rst_normalization is None:
        ...
        square_norm = 1.0 / np.einsum(
            "nN->N", all_points_coeffs * np.conj(all_points_coeffs)
        )
        coeffs = np.einsum("nN,cN,N->cn", all_points_coeffs, weights, square_norm)
...
def get_channel_weights(channel, nh, elements, real_locs, backbone_mask=None):
    ...
    # AAs should be all atoms on the BACKBONE of the AA that matches
        ...
        return np.array(
            np.logical_and(nh["res_ids"][:, 0][real_locs] == channel, backbone_mask),
            dtype=float,
        )
        ...
        return np.array(
            np.logical_and(
                np.logical_and.reduce(
                    [nh["res_ids"][:, 0][real_locs] != aa for aa in AAS]
                ),
                backbone_mask,
            ),
            dtype=float,
...
def get_hologram(
    ...
    real_locs = np.logical_and(
        atom_names != EMPTY_ATOM_NAME, nh["coords"][:, 0] <= r_max
    )
    ...
    if keep_zeros:
        dt = np.dtype(
            [
                (str(l), "complex64", (num_combi_channels, 2 * l + 1))
                for l in range(L_max + 1)
            ]
        )
        ...
        dt = np.dtype(
            [
                (str(l), "complex64", (nmax_per_l[l] * len(channels), 2 * l + 1))
                for l in range(L_max + 1)
            ]
        )
...
def get_frame(nh):
    try:
        ...
        central_CA_coords = np.array(
            [0.0, 0.0, 0.0]
        )  # since we centered the neighborhood on the alpha carbon
        central_N_coords = np.squeeze(
            cartesian_coords[central_res][nh["atom_names"][central_res] == N]
        )
        central_C_coords = np.squeeze(
            cartesian_coords[central_res][nh["atom_names"][central_res] == C]
        )
        logger.info(
            "central_*_coords.shape:",
            central_N_coords.shape,
        ...
        logger.exception(e)
        logger.warn("No central residue (or other unwanted error).")
        frame = None
    return frame